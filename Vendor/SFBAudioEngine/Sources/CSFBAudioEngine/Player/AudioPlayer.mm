...
/// The minimum ring buffer capacity
constexpr std::size_t minimumRingBufferCapacity = 16384;
/// The target ring buffer duration in seconds
constexpr double ringBufferDuration = 0.2;
...
std::size_t ringBufferCapacityForSampleRate(double sampleRate) noexcept {
    if (!std::isfinite(sampleRate) || sampleRate <= 0) {
        return minimumRingBufferCapacity;
    }

    return std::max(minimumRingBufferCapacity, static_cast<std::size_t>(std::ceil(sampleRate * ringBufferDuration)));
}
...
Float64 currentAudioQueueDeviceSampleRate(AVAudioSession *audioSession) noexcept {
    ...
    streamDescription.mSampleRate = audioSession.preferredSampleRate;
    ...
    streamDescription.mFormatFlags = kLinearPCMFormatFlagIsSignedInteger | kLinearPCMFormatFlagIsBigEndian |
                                     kLinearPCMFormatFlagIsPacked;

    const auto status = AudioQueueNewOutput(&streamDescription, [](void *, AudioQueueRef, AudioQueueBufferRef) {},
                                            nullptr, nullptr, kCFRunLoopCommonModes, 0, &audioQueue);
    if (status != noErr || audioQueue == nullptr) {
        return audioSession.sampleRate;
    }

    Float64 deviceSampleRate = 0;
    UInt32 size = sizeof(deviceSampleRate);
    const auto propertyStatus = AudioQueueGetProperty(audioQueue, kAudioQueueDeviceProperty_SampleRate,
                                                      &deviceSampleRate, &size);
    ...
    AudioQueueDispose(audioQueue, true);
    return deviceSampleRate != 0 ? deviceSampleRate : audioSession.sampleRate;
}
#endif /* TARGET_OS_IPHONE */
...
sfb::AudioPlayer::AudioPlayer() {
    ...
    const auto capacity = ringBufferCapacityForSampleRate(format.sampleRate);
    if (!audioRingBuffer_.allocate(*(format.streamDescription), capacity)) {
        os_log_error(log_,
                     "Unable to create audio ring buffer: spsc::AudioRingBuffer::allocate failed with format "
                     "%{public}@ and capacity %zu",
                     SFBASBDFormatDescription(format.streamDescription), capacity);
...
bool sfb::AudioPlayer::pause() noexcept {
    auto wasPlaying = false;
    {
        ...
        wasPlaying = bits::has_all(prevFlags, Flags::engineIsRunning | Flags::isPlaying);
        if (!wasPlaying) {
            return false;
        }
...
bool sfb::AudioPlayer::resume() noexcept {
    ...
    {
        std::lock_guard lock{engineMutex_};
        const auto prevFlags = loadFlags();
        wasPaused = bits::is_set_and_is_clear(prevFlags, Flags::engineIsRunning, Flags::isPlaying);
        if (!wasPaused) {
            return false;
        }
...
bool sfb::AudioPlayer::togglePlayPause(NSError **error) noexcept {
    SFBAudioPlayerPlaybackState playbackState;
    {
        ...
        // Currently stopped or paused, transition to playing
        if (bits::is_clear(flags, Flags::engineIsRunning) || bits::is_clear(flags, Flags::isPlaying)) {
            if (!engine_.isRunning) {
                if (NSError *startError = nil; ![engine_ startAndReturnError:&startError]) {
                    os_log_error(log_, "Error starting AVAudioEngine: %{public}@", startError);
                    if (bits::is_clear(flags, Flags::engineIsRunning)) {
                        clearFlags(Flags::engineIsRunning | Flags::isPlaying);
                    }
                    if (error != nullptr) {
                        *error = startError;
            ...
            setFlags(Flags::engineIsRunning | Flags::isPlaying);
            playbackState = SFBAudioPlayerPlaybackStatePlaying;
        } else {
            if (engine_.isRunning) {
                [engine_ pause];
            }
            clearFlags(Flags::isPlaying);
...
void sfb::AudioPlayer::processDecoders(std::stop_token stoken) noexcept {
    ...
    while (!stoken.stop_requested()) {
        ...
        if (decoderState != nullptr && decoderState->isSeekRequested()) {
            // Mute until the seek is complete and the ring buffer is refilled
            setFlags(Flags::isMuted);
...
void sfb::AudioPlayer::handleAudioSessionInterruption(NSDictionary *userInfo) noexcept {
    ...
    case AVAudioSessionInterruptionTypeEnded: {
...

        {
            ...
            if (bits::has_all(preInterruptState, Flags::engineIsRunning | Flags::isPlaying)) {
                if (NSError *startError = nil; ![engine_ startAndReturnError:&startError]) {
                    os_log_error(log_, "Error starting AVAudioEngine: %{public}@", startError);
                    lock.unlock();
                    if (__strong id<SFBAudioPlayerDelegate> delegate = player_.delegate;
...
bool sfb::AudioPlayer::stopEngineIfRunning() noexcept {
    ...
    return bits::is_set(prevFlags, Flags::engineIsRunning);
}

#if TARGET_OS_IPHONE
bool sfb::AudioPlayer::setPreferredAudioSessionSampleRateForFormat(AVAudioFormat *format, NSError **error) noexcept {
#if DEBUG
    assert(format != nil);
    assert(format.isStandard);
#endif /* DEBUG */

    const auto sampleRatesAreEquivalent = [](double lhs, double rhs) noexcept {
        return std::abs(lhs - rhs) < sampleRateTolerance;
    };

    const auto preferredSampleRate = format.sampleRate;
    AVAudioSession *audioSession = AVAudioSession.sharedInstance;

    if (!sampleRatesAreEquivalent(audioSession.preferredSampleRate, preferredSampleRate)) {
        os_log_debug(log_, "Setting preferred AVAudioSession sample rate to %g Hz", preferredSampleRate);

        if (NSError *sampleRateError = nil; ![audioSession setPreferredSampleRate:preferredSampleRate error:&sampleRateError]) {
            os_log_error(log_, "Error setting preferred AVAudioSession sample rate to %g Hz: %{public}@",
                         preferredSampleRate, sampleRateError);
            if (error != nullptr) {
    ...
    const auto actualSampleRate = currentAudioQueueDeviceSampleRate(audioSession);
    if (!sampleRatesAreEquivalent(actualSampleRate, preferredSampleRate)) {
        os_log_debug(log_, "Preferred AVAudioSession sample rate is %g Hz; actual device sample rate is %g Hz",
...
#if TARGET_OS_IPHONE
    if (!setPreferredAudioSessionSampleRateForFormat(format, error)) {
        return false;
    }
#endif /* TARGET_OS_IPHONE */
