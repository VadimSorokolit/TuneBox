...
- (void)addMetadataFromTagLibID3v2Tag:(const TagLib::ID3v2::Tag *)tag {
    ...
    if (auto frameList = tag->frameListMap()["TPE1"]; !frameList.isEmpty()) {
        NSMutableArray<NSString *> *artists = [NSMutableArray array];
        for (const TagLib::String &value : frameList.front()->toStringList()) {
    ...
    if (auto frameList = tag->frameListMap()["TCON"]; !frameList.isEmpty()) {
        NSMutableArray<NSString *> *genres = [NSMutableArray array];
        if (auto *frame = dynamic_cast<TagLib::ID3v2::TextIdentificationFrame *>(frameList.front()); frame != nullptr) {
            for (TagLib::String value : frame->fieldList()) {
                if (value.isEmpty()) {
                    continue;
                }
                bool ok = false;
                ...
                if (ok && number >= 0 && number <= 255) {
                    value = TagLib::ID3v1::genre(number);
                }
                NSString *stringValue = [NSString stringWithUTF8String:value.toCString(true)];
                if (stringValue != nil && ![genres containsObject:stringValue]) {
...
void sfb::setID3v2TagFromMetadata(SFBAudioMetadata *metadata, TagLib::ID3v2::Tag *tag, bool setAlbumArt) {
    ...
    // if (NSArray<NSString *> *composer = metadata.composer; composer != nil) {
    ...
    // if (NSArray<NSString *> *albumArtist = metadata.albumArtist; albumArtist != nil) {
    ...
    // MusicBrainz
    removeAllTXXXFramesWithDescription("MusicBrainz Album Id");
    ...
    removeAllTXXXFramesWithDescription("MusicBrainz Track Id");
    ...
    // if (NSArray<NSString *> *artistSortOrder = metadata.artistSortOrder; artistSortOrder != nil) {
    ...
    // if (NSArray<NSString *> *albumArtistSortOrder = metadata.albumArtistSortOrder; albumArtistSortOrder != nil) {
    ...
    // if (NSArray<NSString *> *composerSortOrder = metadata.composerSortOrder; composerSortOrder != nil) {
    ...
    // Write TXXX frames
    removeAllTXXXFramesWithDescription("REPLAYGAIN_TRACK_GAIN");
    removeAllTXXXFramesWithDescription("replaygain_track_gain");

    removeAllTXXXFramesWithDescription("REPLAYGAIN_TRACK_PEAK");
    removeAllTXXXFramesWithDescription("replaygain_track_peak");

    removeAllTXXXFramesWithDescription("REPLAYGAIN_ALBUM_GAIN");
    removeAllTXXXFramesWithDescription("replaygain_album_gain");

    removeAllTXXXFramesWithDescription("REPLAYGAIN_ALBUM_PEAK");
    removeAllTXXXFramesWithDescription("replaygain_album_peak");
