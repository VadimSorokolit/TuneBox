...
- (void)addMetadataFromTagLibAPETag:(const TagLib::APE::Tag *)tag {
    ...
    for (auto iterator : tag->itemListMap()) {
        ...
        if (auto type = item.type(); type == TagLib::APE::Item::Text) {
            ...
            for (const TagLib::String &itemValue : item.values()) {
                NSString *stringValue = [NSString stringWithUTF8String:itemValue.toCString(true)];
                if (stringValue != nil) {
            ...
            if ([key caseInsensitiveCompare:@"ALBUM"] == NSOrderedSame) {
                ...
                self.artist = values;
                       ...
                       [key caseInsensitiveCompare:@"ALBUM ARTIST"] == NSOrderedSame) {
