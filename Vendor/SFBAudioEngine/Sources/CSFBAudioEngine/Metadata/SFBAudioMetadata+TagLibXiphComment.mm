...
- (void)addMetadataFromTagLibXiphComment:(const TagLib::Ogg::XiphComment *)tag {
    ...
    for (auto it : tag->fieldListMap()) {
        ...
        for (const TagLib::String &fieldValue : it.second) {
            NSString *stringValue = [NSString stringWithUTF8String:fieldValue.toCString(true)];
            if (stringValue != nil) {
        ...
        } else if ([key caseInsensitiveCompare:@"PARTOFACOMPILATION"] == NSOrderedSame) {
            self.compilation = value != nil ? @(value.boolValue) : nil;
