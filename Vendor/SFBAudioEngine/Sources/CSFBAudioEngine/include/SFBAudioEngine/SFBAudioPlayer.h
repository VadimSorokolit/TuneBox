...
typedef NS_ENUM(NSUInteger, SFBAudioPlayerPlaybackState) {
    ...
    /// The player is paused and not rendering audio
...
/// Pauses audio rendering and pauses the `AVAudioEngine`
/// - note: If the current playback state is not `SFBAudioPlayerPlaybackStatePlaying` this method has no effect
/// - returns: `YES` if playback transitioned to paused
- (BOOL)pause;
/// Starts the `AVAudioEngine` and resumes audio rendering
...
/// `YES` if playback is paused
@property(nonatomic, readonly) BOOL isPaused;
/// `YES` if playback is stopped
