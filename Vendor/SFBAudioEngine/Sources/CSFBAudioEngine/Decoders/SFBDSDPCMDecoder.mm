...
- (BOOL)openReturningError:(NSError **)error {
    ...
    // TuneBox: allow DSD64/128/256 (+ 48 kHz variants) for PCM conversion on iOS.
    const Float64 sampleRate = asbd->mSampleRate;
    const bool isSupportedPCMConversionRate =
        sampleRate == static_cast<Float64>(kSFBSampleRateDSD64) ||
        sampleRate == static_cast<Float64>(kSFBSampleRateDSD128) ||
        sampleRate == static_cast<Float64>(kSFBSampleRateDSD256) ||
        sampleRate == static_cast<Float64>(kSFBSampleRateDSD64Variant) ||
        sampleRate == static_cast<Float64>(kSFBSampleRateDSD128Variant) ||
        sampleRate == static_cast<Float64>(kSFBSampleRateDSD256Variant);

    if (!isSupportedPCMConversionRate) {
        os_log_error(gSFBAudioDecoderLog, "Unsupported DSD sample rate for PCM conversion: %g", sampleRate);
        if (error != nullptr) {
            NSMutableDictionary *userInfo = [NSMutableDictionary
